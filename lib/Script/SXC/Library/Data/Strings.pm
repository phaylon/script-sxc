...
CLASS->add_procedure('string?',
    firstclass => sub { 
        Script::SXC::Runtime::Validation->runtime_arg_count_assertion('string?', [@_], min => 1);
        for (@_) { 
...
CLASS->add_procedure('join',
    firstclass  => sub {
        Script::SXC::Runtime::Validation->runtime_arg_count_assertion('join', [@_], min => 2, max => 2);
        Script::SXC::Runtime::Validation->runtime_type_assertion($_[1], 'list', 'join expects a list as second argument');
        my ($sep, $ls) = @_;
        return join $sep, @$ls;
    },
    inline_fc   => 1,
...
CLASS->add_procedure('length',
    firstclass => sub {
        Script::SXC::Runtime::Validation->runtime_arg_count_assertion('length', [@_], min => 1, max => 1);
        return length shift;
    },
    inline_fc   => 1,
    runtime_req => ['Validation'],
...
CLASS->add_procedure('sprintf',
    firstclass => sub {
        Script::SXC::Runtime::Validation->runtime_arg_count_assertion('sprintf', [@_], min => 2);
        my ($format, @args) = @_;
        no warnings;
    ...
    inliner     => method (Object :$compiler!, Object :$env!, ArrayRef :$exprs!, :$error_cb!, :$name!) {
        ...
        return CompiledValue->new(typehint => 'string', content => sprintf 'sprintf(%s)',
            join ', ', map { $_->compile($compiler, $env, to_string => 1) } @$exprs,
        );
    },
);
